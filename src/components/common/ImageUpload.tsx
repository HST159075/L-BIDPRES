...
export function ImageUpload({
  ...
  accept = "image", className, disabled, label,
}: ImageUploadProps) {
      ...
      >
        ...
        <div className="text-center">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            {accept === "image" ? "PNG, JPG, WEBP — Max 5MB" : "MP4, MOV — Max 50MB"}
          </p>
        </div>
        <input
          type="file"
...
export function MultiImageUpload({ values, onChange, max = 10, className }: MultiImageUploadProps) {
  const [isUploading, setIsUploading] = useState(false);
      ...
      <div className="grid grid-cols-4 sm:grid-cols-5 gap-2">
        ...
        {values.length < max && (
          <label className="aspect-square border-2 border-dashed border-border rounded-xl flex flex-col items-center justify-center cursor-pointer hover:border-bid-500 hover:bg-bid-500/5 transition-colors">
            ...
            <input type="file" accept="image/*" multiple disabled={isUploading} onChange={handleChange} className="sr-only" />
          </label>
        )}
      </div>
      <p className="text-xs text-muted-foreground">{values.length}/{max} photos (min 2 required)</p>
    </div>
  );
}
