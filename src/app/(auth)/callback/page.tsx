      ...
      try {
        ...
        if (session?.data?.user) {
          ...
          const mappedUser: User = {
            ...
            phoneVerified: !!(raw as typeof raw & { phoneVerified?: boolean })
              .phoneVerified,
            role: (raw as typeof raw & { role?: UserRole }).role ?? "buyer",
            avatar: raw.image ?? null,
            purchaseCount: Number(
              (raw as typeof raw & { purchaseCount?: number }).purchaseCount ??
                0,
            ),
            strikeCount: Number(
              (raw as typeof raw & { strikeCount?: number }).strikeCount ?? 0,
            ),
            isBanned: Boolean(
              (raw as typeof raw & { isBanned?: boolean }).isBanned,
            ),
            createdAt:
