...
interface Bid {
  ...
  status: string;
  createdAt: string;
  auction?: { id?: string; listing?: { title?: string } };
...
export default function BuyerDashboardPage() {
  ...
  const [confirmingAuctionId, setConfirmingAuctionId] = useState<string | null>(null);
  ...
  const handleConfirmDelivery = async (auctionId: string) => {
    setConfirmingAuctionId(auctionId);
    try {
      await paymentService.confirmDelivery(auctionId);
      showSuccess("Delivery confirmed! Payment released to seller.");
      setRefreshKey((prev) => prev + 1);
    } catch (err) {
      showError(err);
    } finally {
      setConfirmingAuctionId(null);
    }
  };
  ...
  const stats = [
    { label: "Total Bids", value: bids.length, icon: Gavel, color: "text-bid-500" },
    { label: "Purchases", value: user?.purchaseCount || 0, icon: ShoppingBag, color: "text-green-500" },
    { label: "Active Bids", value: bids.filter((b) => b.status === "active").length, icon: TrendingUp, color: "text-blue-500" },
          ...
          <ScrollReveal className="mt-8 mb-10">
            <div className="flex items-start justify-between">
              <div>
                ...
                <span className="inline-block mt-2 px-3 py-1 rounded-full text-xs font-medium bg-bid-500/10 text-bid-500 capitalize">{user?.role}</span>
              </div>
              {canApplyForSeller && (
                <Link href={ROUTES.sellerApply} className="flex items-center gap-2 px-4 py-2.5 bg-bid-500 text-white text-sm font-semibold rounded-xl hover:bg-bid-600 transition-colors">
            ...
            <ScrollReveal className="mb-10">
              <div className="bg-card border border-border rounded-2xl p-6">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-sm text-muted-foreground">{user.purchaseCount}/5 purchases</span>
                </div>
                <div className="h-2 bg-muted rounded-full overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${Math.min((user.purchaseCount / 5) * 100, 100)}%` }}
                    transition={{ duration: 1, ease: "easeOut" }}
                    className="h-full bg-bid-500 rounded-full"
                  />
                </div>
                <p className="text-xs text-muted-foreground mt-2">
                  {user.purchaseCount >= 5 ? "You can now apply for a seller account!" : `${5 - user.purchaseCount} more purchase(s) to unlock seller.`}
                </p>
              </div>
            </ScrollReveal>
          )}
          ...
          <ScrollReveal>
            <div className="bg-card border border-border rounded-2xl p-6">
              <div className="flex items-center justify-between mb-5">
                ...
                <Link href={ROUTES.buyerBids} className="text-sm text-bid-500 hover:underline">View all</Link>
              </div>
              {bidsLoading ? (
                ...
                <div className="text-center py-10 text-muted-foreground">
                  ...
                  <Link href={ROUTES.auctions} className="text-sm text-bid-500 hover:underline mt-1 block">Browse auctions</Link>
                </div>
              ) : (
                <div className="space-y-3">
                  {bids.slice(0, 5).map((bid) => (
                    <div key={bid.id} className="flex items-center gap-4 p-3 bg-muted/50 rounded-xl">
                      <div className={`w-2.5 h-2.5 rounded-full shrink-0 ${
                        bid.status === "active" ? "bg-green-500" :
                        bid.status === "won" ? "bg-blue-500" :
                        bid.status === "outbid" ? "bg-red-500" : "bg-gray-400"
                      }`} />
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium truncate">{bid.auction?.listing?.title || "Auction"}</p>
                        <p className="text-xs text-muted-foreground flex items-center gap-1">
                          <Clock className="w-3 h-3" /> {formatTimeAgo(bid.createdAt)}
                        </p>
                      </div>
                      <div className="text-right shrink-0">
                        <p className="font-bold text-sm">{formatPriceEn(bid.amount)}</p>
