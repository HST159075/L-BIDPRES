...
export default function AdminDashboardPage() {
  ...
  const stats: StatItem[] = [
    ...
    },
    {
      ...
      value: formatPriceEn(Number(analytics.totalRevenue) || 0),
      icon: DollarSign,
      ...
      <div className="min-h-screen bg-[var(--color-background)]">
        ...
        <main className="pt-24 pb-16 max-w-6xl mx-auto px-4 sm:px-6">
          <ScrollReveal className="mb-10">
            <p className="text-[var(--color-muted-foreground)] text-sm font-medium uppercase tracking-wider">
              Admin Panel
            </p>
            ...
            {stats.map((stat) => (
              <StaggerItem key={stat.label}>
                <div className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-2xl p-5 hover:shadow-md transition-shadow">
                  <div
                    className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 ${stat.bg} ${stat.color}`}
                  >
                  ...
                  <div>
                    <p className="text-2xl font-bold tabular-nums">
                      {loading ? (
                        <span className="animate-pulse bg-[var(--color-muted)] rounded w-16 h-8 block" />
                      ) : (
                        stat.value
                      )}
                    </p>
          ...
          <ScrollReveal>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <ActionCard
                ...
                icon={Users}
                title="Manage Users"
                description="Strike, ban, or unban users"
              />
              <ActionCard
              ...
              <div className="flex items-center justify-between p-5 bg-amber-500/10 border border-amber-500/20 rounded-2xl">
                <div className="flex items-center gap-4">
                  ...
                  <p className="text-sm font-medium">
                    {analytics.pendingApps} seller application
                    {(analytics.pendingApps ?? 0) > 1 ? "s" : ""} waiting for
                    review
                  </p>
                </div>
                <Link
                  href={ROUTES.adminApplications}
                  className="px-5 py-2 bg-amber-500 text-white text-sm font-bold rounded-xl hover:bg-amber-600 transition-all"
                >
                  Review Now
                </Link>
              </div>
...
function ActionCard({ href, icon: Icon, title, description }: ActionCardProps) {
  return (
    <Link href={href}>
      <motion.div
        whileHover={{ y: -4 }}
        className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-2xl p-6 hover:border-[var(--color-bid-500)]/50 transition-all group shadow-sm"
      >
